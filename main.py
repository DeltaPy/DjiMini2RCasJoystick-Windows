...
parser = argparse.ArgumentParser(
    description='DJI Mini 2 RC (also known as RC-N1, RCS231, WM161b-RC-N1, RCN1) <-> vGamePad')
...
# Default values (min 364, center 1024, max 1684)


def parseInput(input, name):
    # I dont know how to map joystick ranges but it works
    output = (int.from_bytes(input, byteorder='little') - 1024) / 660
    return output
...
def threaded_function():
    while(True):
        gamepad.right_joystick_float(
            x_value_float=st["rh"], y_value_float=st["rv"])
        gamepad.left_joystick_float(
            x_value_float=st["lh"], y_value_float=st["lv"])
        gamepad.update()
